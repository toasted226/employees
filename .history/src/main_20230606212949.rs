...
fn add_employees_to_department(names: &Vec<String>, department: &str, departmentsMap: &mut HashMap<String, Vec<String>>) {
    for name in names {
        let v: Vec<String> = Vec::new();
        let department_names = departmentsMap.entry(department.to_string()).or_insert(v);
